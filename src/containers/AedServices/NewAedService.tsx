...
const NewAedService = forwardRef<NewAedHandle, NewAedProps>(({data, closeModal}, ref) => {
    ...
    const {getAllRepairTypes} = new RepairType();
    ...
    const [searchRepairType, setSearchRepairType] = useState("");
    ...
    const debouncedRepairTypeSearch = React.useRef(
        debounce((val) => {
            setSearchRepairType(val);
        }, 500)
    ).current;
    ...
    const {data: repairTypeOptions = [], isLoading: isRepairTypeLoading} = useQuery<RepairTypeType[]>(
        ["repair-type-search", searchRepairType],
        async () => {
            const res = await getAllRepairTypes(10, 0, `contains(title,'${searchRepairType}')`);
            return res.data.data.map((r: any) => ({
                id: r.id,
                title: r.title,
            }));
            ...
            >
                {(formik) => (
                    <form onSubmit={formik.handleSubmit}>
                        ...
                        <br/>
                        {
                            (data.correctiveActionGroup === "Repair" || data.correctiveActionGroup === "Training") && (
                               <>
                                   {/* Call Date */}
                        ...
                        {
                            (data.correctiveActionGroup !== "Pm" && data.correctiveActionGroup !== "Training") && (
                                <>
                                    {/* Non Conformity */}
                        ...
                        {
                            (data.correctiveActionGroup === "Repair" || data.correctiveActionGroup === "Recall") && (
                                <>
                                    {/* Non Conformity */}
                                    <InputLabel htmlFor="repairTypeId">
                                        <Typography>📐 Repair Type</Typography>
                                    </InputLabel>
                                    <Autocomplete
                                        disablePortal
                                        options={repairTypeOptions}
                                        loading={isRepairTypeLoading}
                                        value={repairType}
                                        getOptionLabel={(option) => option?.title ?? ""}
                                        onInputChange={(_, value) => {
                                            debouncedRepairTypeSearch(value);
                                        }}
                                        onChange={(_, newValue) => {
                                            formik.setFieldValue("repairTypeId", newValue ? newValue.id : "");
                                            setRepairType(newValue);
                                        }}
                                        renderInput={(params) => <StyledTextField {...params}
                                                                                  placeholder="Search Repair Type..."/>}
                                    />
                                    {formik.errors.repairTypeId && formik.touched.repairTypeId && (
                                        <Typography color="error" variant="caption">
                                            {formik.errors.repairTypeId}
                                        </Typography>
                                    )}
                                </>
                            )
